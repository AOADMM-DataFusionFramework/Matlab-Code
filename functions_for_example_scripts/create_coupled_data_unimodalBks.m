function  [X, A, Delta,sigma] = create_coupled_data_unimodalBks(varargin)
    ...
    for n = modes{p}
        if lin_coupled_modes(n) == 0 % this mode is not coupled
            ...
            if (strcmp(model{p},'PAR2') && 2 == find(modes{p}==n))
                %UNIMODAL PARAFAC2-components
                A{n} = cell(length(sz{n}),1);
                AA = zeros(sz{n}(1),length(lambdas{p}));
                for r=1:length(lambdas{p})
                    AA(:,r) = normpdf(linspace(-10,10,sz{n}(1)),-7+(r-1)*7, 0.8);
                end
                A{n}{1} = AA;
                for k=2:length(sz{n})
