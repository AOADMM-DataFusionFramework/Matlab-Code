function  [X, A, Delta,sigma] = create_CP_data_example10piecewiseconstant(varargin)
...
%%%%%%%%%%%%
% first mode with piecewise constant factor vectors
for r=1:length(lambdas{p})
    jumps = [1;sort(randi(sz{1},4,1));sz{1}];
    values = -1 +2*rand(5,1);
    for i=1:5
        A{1}(jumps(i):jumps(i+1),r)= values(i);
    end
end
if normalize_columns
    for r=1:length(lambdas{p})
        A{1}(:,r)=A{1}(:,r)/norm(A{1}(:,r));
    end
end
%%%%%%%%%%%%
