...
sz     = {20,30,40, 20,30*ones(1,40),40}; %size of each mode
...
noise = [0,0]; %level of noise, for gaussian noise only!
distr_data = {@(x,y) rand(x,y)+0.1, @(x,y) rand(x,y),@(x,y) rand(x,y),@(x,y) rand(x,y),@(x,y) randn(x,y),@(x,y) rand(x,y)+0.1}; % function handle of distribution of data within each factor matrix /or Delta if linearly coupled, x,y are the size inputs %coupled modes need to have same distribution! If not, just the first one will be considered
...
model{2} = 'PAR2';
%% specify couplings
coupling.lin_coupled_modes = [1 0 0 0 0 1]; % which modes are coupled, coupled modes get the same number (0: uncoupled)
...
coupling.coupl_trafo_matrices{6} = zeros(20,40);
for i=1:20
  coupling.coupl_trafo_matrices{6}(i,i+(i-1)) = 1; %take every second entry
end
...
init_options.distr = distr_data;%{@(x,y) truncated_randn(x,y), @(x,y) randn(x,y),@(x,y) randn(x,y),@(x,y) truncated_randn(x,y),@(x,y) truncated_randn(x,y),@(x,y) rand(x,y)}; % distribution of the initial factor matrices and their auxiliary variables
...
%% set constraints
constrained_modes = [1 1 1 1 0 1]; % 1 if the mode is constrained in some way, 0 otherwise, put the same for coupled modes!
