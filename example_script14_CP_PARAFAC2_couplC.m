...
distr_data = {@(x,y) rand(x,y)+0.1, @(x,y) randn(x,y),@(x,y) randn(x,y),@(x,y) randn(x,y),@(x,y) randn(x,y),@(x,y) rand(x,y)+0.1}; % function handle of distribution of data within each factor matrix /or Delta if linearly coupled, x,y are the size inputs %coupled modes need to have same distribution! If not, just the first one will be considered
...
%% set constraints
constrained_modes = [1 0 0 0 0 1]; % 1 if the mode is constrained in some way, 0 otherwise, put the same for coupled modes!
