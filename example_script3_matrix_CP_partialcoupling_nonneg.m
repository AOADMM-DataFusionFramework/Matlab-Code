...
coupling.coupling_type = [4]; % for each coupling number in the array lin_coupled_modes, set the coupling type: 0 exact coupling, 1: HC=Delta, 2: CH=Delta, 3: C=HDelta, 4: C=DeltaH
...
prox_operators = cell(5,1); % cell array of length number of modes containing the function handles of proximal operator for each mode, empty if no constraint
...
prox_operators{5} = @(x,rho) prox_normalized_nonneg(x); % non-negativity
...
%Z.reg_func = cell(5,1); % cell array of length number of modes containing the function handles of regularization functions for each mode, empty if no regularization; function should operate on the whole matrix
%Z.reg_func{1} =  @(x) 
